 ...
 *
 * Usage:
 *   AGREEMENTS_API_BASE_URL=... EVAL_API_KEY=... node test/e2e-eval.mjs
 *
...
const mcpServer = await startAgreementsMcpHttpServer({ port: 0, host: '127.0.0.1', baseUrl });
...
const validation = await client.callTool({ name: 'validate_agreement', arguments: { agreement } });
...
const preflight = await client.callTool({
  ...
  arguments: { agreement, participants },
});
...
const list = await client.callTool({ name: 'list_agreements', arguments: { limit: 3 } });
