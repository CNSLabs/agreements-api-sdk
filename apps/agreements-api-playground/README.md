...
- `VITE_AGREEMENTS_API_BASE_URL` is optional and overrides the environment host mapping.
- On localhost, leaving `VITE_AGREEMENTS_API_BASE_URL` empty defaults the playground to the Vite dev server origin so `/partner-api`, `/auth-api`, `/agreements-api`, `/notifications-api`, and `/agreements` can be proxied through `http://localhost:8080`.
- `VITE_AGREEMENTS_API_BASE_URL` should be the gateway origin only. Do not append `/partner-api` or `/partner-api/v0`.
...
- `VITE_AGREEMENTS_API_BASE_URL`: optional advanced override for local proxies or custom gateways
