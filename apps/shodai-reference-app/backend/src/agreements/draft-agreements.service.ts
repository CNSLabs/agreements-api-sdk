...
export class DraftAgreementsService {
  ...
  async createDraft(body: any, user: any) {
    ...
    const templateId = await this.requireFrontendTemplateId(body?.templateId);
    const agreement = await this.requireCatalogTemplate(templateId);
  ...
  private async requireFrontendTemplateId(value: unknown) {
    const rawTemplateId = String(value || '').trim();
    if (!rawTemplateId) {
      throw new BadRequestException('Request body must include a valid templateId');
    }
  ...
  private async requireCatalogTemplate(templateId: string) {
    try {
      return await this.catalog.getTemplate(templateId);
    } catch (error) {
