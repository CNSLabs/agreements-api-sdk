# Third-Party Local Setup

This repo runs as a standalone agreements app. It does not require any separate `auth-api`, `agreements-api`, nginx, notifications services, or local external API.
...
Skip automatic seeding in `dev:start`:

```sh
SKIP_TEMPLATE_ACCESS_SEED=1 pnpm dev:start
```
